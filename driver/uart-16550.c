...
#define ENABLE_UART_INTERRUPT 0 //use uart irq
...
#if ENABLE_UART_INTERRUPT == 1
void T5UIC2_recData_irq(void);
void uart0_irq(void *arg)
{
	virtual_addr_t addr = 0x01c25000;
	u32_t s = read32(addr + 0x08);
	if ((s & 0xff) == 0xc4) //rec irq
	{
		T5UIC2_recData_irq();
		//read32(addr + 0x00);
	}
...
static int uart_16550_set(struct uart_t * uart, int baud, int data, int parity, int stop)
{
	...
	/*enable interrupt*/
	if (uart->name == UART0)
		request_irq(IRQ_UART0, uart0_irq, 0); //register int
	else if (uart->name == UART1)
		request_irq(IRQ_UART1, uart1_irq, 0); //register int
	else if (uart->name == UART2)
		request_irq(IRQ_UART2, uart2_irq, 0); //register int
#endif
...
static ssize_t uart_16550_read(struct uart_t * uart, u8_t * buf, size_t count)
{
	...
	if(usb_com_open)
	{
		for(i = 0; i < count; i++, usbbuf_cnt++)
		{
			buf[i] = usbbuf[usbbuf_cnt];
		}	
		return i;
	}
	else
...
static ssize_t uart_16550_write(struct uart_t * uart, const u8_t * buf, size_t count)
{
	...
	if(usb_com_open)
	{
		usb_cdc_out_ep_callback((unsigned char *)buf, &j);
		return count;
	}
	else
	{
...
u8_t uart_16550_rec_fifo_cnt(struct uart_t * uart)
{
	struct uart_16550_pdata_t * pdat = (struct uart_16550_pdata_t *)uart->priv;
	u32_t sta;
	if(usb_com_open)
	{
		usb_cdc_in_ep_callback(usbbuf, (int *)&sta);
		usbbuf_cnt = 0;
		return (u8_t)sta;
	}
	else
	{
		sta = read32(pdat->virt + UART_RFL);
		return (u8_t)sta;
	}
}
...
struct uart_t * uart_16550_init(u8_t port, int baud, int data, int parity, int stop)
{
	...
	reset_f1c100s_assert(&re_v, pdat->reset);
	delay_ms(10);
	...
	if (port == UART0)
	{
		...
		gpio_f1c100s_set_pull(&GPIO_PE, pdat->rxd, GPIO_PULL_NONE);
		gpio_f1c100s_set_cfg(&GPIO_PE, pdat->txd, pdat->txdcfg); //TX
		gpio_f1c100s_set_pull(&GPIO_PE, pdat->txd, GPIO_PULL_NONE);
	}
	else if (port == UART1)
	{
		gpio_f1c100s_set_cfg(&GPIO_PA, pdat->rxd, pdat->rxdcfg); //RX
		gpio_f1c100s_set_pull(&GPIO_PA, pdat->rxd, GPIO_PULL_NONE);
		gpio_f1c100s_set_cfg(&GPIO_PA, pdat->txd, pdat->txdcfg); //TX
		gpio_f1c100s_set_pull(&GPIO_PA, pdat->txd, GPIO_PULL_NONE);
	}
	else if (port == UART2)
	{
		gpio_f1c100s_set_cfg(&GPIO_PE, pdat->rxd, pdat->rxdcfg); //RX
...
#if ENABLE_UART_INTERRUPT == 1
	/*use irq rec*/
	write32(pdat->virt + UART_IER, 0x01);
	write32(pdat->virt + UART_FCR, 0x37);
...
void uart_16550_fifo_reset(struct uart_t * uart)
{
	struct uart_16550_pdata_t * pdat = (struct uart_16550_pdata_t *)uart->priv;

	write32(pdat->virt + UART_FCR, 0xf7);
	delay_ms(1); //wait ready
}
