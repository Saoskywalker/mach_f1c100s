...
// #define sys_sd_debug(...) printf(__VA_ARGS__)
#define sys_sd_debug(...)
...
void cpu_flush_dcache_icache(unsigned long start, unsigned long size); //F1C100S没有写缓冲, cache操作flush
...
int fmiSDCheckSector(u32 uSector, u32 uBufcnt)
{
	...
	{
		sys_sd_debug("ERROR: Fail to access invalid sector number %d from SD card !!\r\n", uSector + uBufcnt - 1);
		sys_sd_debug("       The max valid sector number for current SD card is %d.\r\n", g_max_valid_sector);
...
int mmc_set_mod_clk(int SDinx, unsigned int hz)
{
	...
	if (n > 3)
	{
		sys_sd_debug("mmc %u error cannot set clock to %u\r\n", SDinx, hz);
		return -1;
	}
...
#if SD_DBG_PRINTF
	sys_sd_debug("mmc %u set [mod-clk req %u]-[parent %u]-[n %u]-[m %u]-[rate %u]\r\n",
...
int mmc_config_clock(int SDinx, unsigned int hz)
{
	...
	{
		sys_sd_debug("err: mmc_clock_disable\r\n");
		return -1;
	}
	//
	...
	{
		sys_sd_debug("err: mmc_set_mod_clk\r\n");
		return -1;
	}
	/* 清内部分频器为0 */
	...
	{
		sys_sd_debug("err: mmc_clock_enable\r\n");
		return -1;
	}
	sys_sd_debug("mmc_config_clock: %d ok...\r\n", hz);
	return 0;
}
...
int mmc_trans_data_by_cpu(int SDinx, struct mmc_data *data)
{
	...
	unsigned int SD_ADDR = 0;
#if SD_DBG_PRINTF
	sys_sd_debug("Trans data by cpu...\r\n");
	if (reading == 1)
		sys_sd_debug("Data Read...\r\n");
	else
		sys_sd_debug("Data Write...\r\n");
#endif
	...
	for (i = 0; i < (byte_cnt >> 2); i++)
	{
		while (read32(SD_ADDR + 0x03c) & status_bit)
		{
			if (!timeout_usecs--)
			{
				sys_sd_debug("Data[%d] timeout BIT=0x%08x...\r\n", i, read32(SD_ADDR + 0x03c));
				return -1;
			}
...
int mmc_trans_data_by_dma(int SDinx, struct mmc_data *data, struct mmc_des_v4p1 *pdes)
{
	...
	unsigned int SD_ADDR = 0;
#if SD_DBG_PRINTF
	sys_sd_debug("Trans data by dma...\r\n");
	...
	//开启cache后刷新缓存
	cpu_flush_dcache_icache((unsigned long)buff, (unsigned long)byte_cnt);
	...
	for (i = 0; i < buff_frag_num; i++, des_idx++)
	{
		...
		//打印表
#if SD_DBG_PRINTF
		sys_sd_debug("frag %d, remain %d, des[%d](%08x): "
	...
	//开启cache后刷新缓存
	cpu_flush_dcache_icache((unsigned long)pdes, sizeof(struct mmc_des_v4p1) * (des_idx + 1));
...
int sunxi_mmc_send_cmd(int SDinx, struct mmc_cmd *cmd, struct mmc_data *data)
{
...
#if SD_DBG_PRINTF
		sys_sd_debug("trans data %d bytes\r\n", bytecnt);
#endif
		if (bytecnt > 64)
		{
			usedma = 1; //DMA使用标志
	...
	if (data)
	{
		timeout_msecs = 120;
#if SD_DBG_PRINTF
		sys_sd_debug("Wait CONNADN_DONE or DATA_OVER\r\n");
#endif
		//
...
#if SD_DBG_PRINTF
		sys_sd_debug("mmc resp[0]=0x%08x\r\n", cmd->response[0]);
		sys_sd_debug("mmc resp[1]=0x%08x\r\n", cmd->response[1]);
		sys_sd_debug("mmc resp[2]=0x%08x\r\n", cmd->response[2]);
		sys_sd_debug("mmc resp[3]=0x%08x\r\n", cmd->response[3]);
#endif
	}
	else //短
	{
...
out:
	...
	if (data && (data->flags & MMC_DATA_READ))
	{
		...
		cpu_flush_dcache_icache((unsigned long)buff_, (unsigned long)byte_cnt_);
#if SD_DBG_PRINTF
		sys_sd_debug("invald cache after read complete\r\n");
#endif
	}
...
void Get_SD_info(int SDinx, struct mmc_cmd *cmd, DISK_DATA_T *_info)
{
	...
	sys_sd_debug("SD_info print...\r\n");
	sys_sd_debug("SD_info->diskSize=[%d]\r\n", _info->diskSize);
	sys_sd_debug("SD_info->totalSectorN=[%d]\r\n", _info->totalSectorN);
	sys_sd_debug("SD_info->sectorSize=[%d]\r\n", _info->sectorSize);
}
/*设置卡到高速模式*/
int CardSwitchToHighSpeed(int SDinx, struct mmc_cmd *cmd)
{
	...
	if (!busy_status0) // function ready
	{
		...
		if (!busy_status1)
			sys_sd_debug("switch into high speed mode !!!\r\n");
...
int SelectCard(int SDinx, struct mmc_cmd *cmd, char CARD_WIDTH)
{
	...
	{
		sys_sd_debug("CARD_TYPE_SD_HIGH \r\n");
		//读SCR寄存器
		//SD数据线默认1位宽度传输
		...
		sys_sd_debug("buf=0x");
		for (int i = 0; i < 8; i++)
			sys_sd_debug("%02x", _card_ucSDHCBuffer[i]);
		sys_sd_debug(" \r\n");
#endif
		//
	...
	sys_sd_debug("SD_BLOCK_SIZE 512 \r\n");
	//设置块长度
...
int SD_Read_in(int SDinx, unsigned int sector, unsigned int count, unsigned char *buff)
{
	...
	if (buff == 0)
	{
		sys_sd_debug("ERROR: fmiSD_Read_in(): uBufcnt cannot be 0!!\r\n");
		return -745435;
	}
	if ((u32)(long)buff & 0x3)
	{
		sys_sd_debug("SD_Read Addr Align Err[%08x] ...\r\n", buff);
		return -474385;
	} //地址对齐检测吗？
...
int SD_Init(int SDinx)
{
	...
	if (returni == 0)																   //SD2.0
	{
		...
		if (cmd.response[3] & 0x40000000)
		{
			...
			sys_sd_debug("TYPE_SD_HIGH...\r\n");
		}
		else
		{
			CARD_TYPE = CARD_TYPE_SD_LOW;
			sys_sd_debug("TYPE_SD_LOW...\r\n");
		}
	}
	else //SD1.0
	{
		...
		if (returni == ETIMEDOUT) // MMC memory
		{
			...
			if (SD_CMD_SEND(SDinx, &cmd, NULL, 1, 0x80ff8000, MMC_RSP_PRESENT | MMC_RSP_CRC) != ETIMEDOUT) // MMC memory
			{
				...
				sys_sd_debug("TYPE_MMC...\r\n");
			}
			else
			{
				CARD_TYPE = CARD_TYPE_UNKNOWN;
				sys_sd_debug("ERR_DEVICE...\r\n");
				return -1;
			}
		}
...
void SD_Demo(void)
{
	...
	if (SD_Init(SDx) == 0)
	{
		...
		for (i = 0; i < 1024; i++)
			Test_Buff[i] = 0;
		//读测试
		sys_sd_debug("sd read\r\n");
		SD_Read_in(SDx, g_max_valid_sector - 1100, 1, Test_Buff);
		//打印
		for (i = 0; i < 512; i += 2)
			sys_sd_debug("Test_Buff[%d]=0x%02x\r\n", i, Test_Buff[i]);
